...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}Starting 06 Live Metrics Service${NC}"
echo -e "${CYAN}========================================${NC}"

# Get the script directory and project root
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_ROOT="$( cd "$SCRIPT_DIR/../.." && pwd )"
SERVICE_DIR="$PROJECT_ROOT/06_Live_Metrics_Service"
...
echo -e "${GREEN}Starting Live Metrics Service on http://127.0.0.1:8010${NC}"
echo -e "${GREEN}GET /health  GET /metrics${NC}"
echo -e "${YELLOW}Press Ctrl+C to stop the server${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""

python3 -m uvicorn live_metrics_service:app --host 127.0.0.1 --port 8010 --reload
